...
def find_nearest_driver(pickup_lat, pickup_lng):
    conn = sqlite3.connect("booking_agent.db")
    cursor = conn.cursor()
    cursor.execute("SELECT name, vehicle, plate, latitude, longitude FROM drivers WHERE available = 1")
    ...
    for name, vehicle, plate, lat, lng in drivers:
...
def post_job(state):
    ...
    driver = find_nearest_driver(pickup_lat, pickup_lng)

    if not driver:
        raise Exception("No available driver with location data")

    driver_name, vehicle, plate = driver
